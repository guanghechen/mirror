...
export async function check_branch_or_commit_exist(branchNameOrCommitId) {
  const args = ['git', 'log', '-1', '--pretty=format:%h %s', branchNameOrCommitId]
  const result = await run_command(args, { silent: true })
  return result !== false
}
...
export async function get_full_commit_id(branchNameOrCommitId) {
  const args = ['git', 'rev-parse', branchNameOrCommitId]
  const result = await run_command(args, { silent: true, quitOnError: true })
  return result
}

export async function get_parent_commit_id(commitId) {
  try {
    const args = ['git', 'rev-parse', `${commitId}^`]
    const result = await run_command(args, { silent: true, quitOnError: true })
    return result
  } catch (_error) {
